...
#     ),
    ...
    numericInput("mu.x",HTML("$\\hat{a}:$"),0),
    
    numericInput("mu.y",HTML("$\\hat{b}:$"),0),
    
    numericInput("se.x",("$SE_\\hat{a}:$"),1),

    textOutput("SEaMessages"),
    
    numericInput("se.y",HTML("$SE_\\hat{b}:$"),1),
    
    textOutput("SEbMessages"),
    
    numericInput("alpha",HTML("$\\alpha:$"),0.05),
       
    textOutput("alphaMessages"),
  
    numericInput("rho",HTML("$\\rho:$"),0),
...
<p>To compute the confidence interval for the mediated effect, $a\\cdot b$, using the distribution of the product of the coefficients method, input the following values into the boxes 
...
#     
    ...
    HTML("<br>"),
    HTML("<b>NOTE:</b> When $\\rho=0$ the standard error for the indirect effect is calculated using $SE_{(\\hat{a}\\hat{b})}=\\sqrt{(a\\,SE(\\hat{b}))^2+(b\\,SE(\\hat{a}))^2+SE(\\hat{a})^2\\,SE(\\hat{b})^2}$.")
